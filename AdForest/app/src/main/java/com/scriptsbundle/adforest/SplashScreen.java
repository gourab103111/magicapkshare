...
public class SplashScreen extends AppCompatActivity {
    ...
    boolean istermsPrivacyOn = false;
    String gmap_lang;
...
// Check for update availability
                ...
                appUpdateInfoTask.addOnSuccessListener(appUpdateInfo -> {
                    if (appUpdateInfo.updateAvailability() == UpdateAvailability.UPDATE_AVAILABLE
                        ...
                        // An update is available, prompt the user to start the update
                        try {
                            appUpdateManager.startUpdateFlowForResult(
                                    appUpdateInfo,
                                    AppUpdateType.FLEXIBLE,
                                    this, // Activity instance
                                    MY_REQUEST_CODE); // Request code for the update flow
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == MY_REQUEST_CODE) {
            if (resultCode != RESULT_OK) {
                // Update flow failed or was cancelled by the user, handle the situation accordingly
                adforest_getSettings();
...
//                alert.show();
    ...
    public void adforest_getSettings() {
        RestService restService =
                UrlController.createService(RestService.class,getApplicationContext());
        try {
                    ...
                    try {
                        if (responseObj.isSuccessful()) {
                            ...
                            if (response.getBoolean("success")) {
                                ...
                                if (jsonObjectSetting.has("is_term_screen_show")) {
                                    istermsPrivacyOn = jsonObjectSetting.getBoolean("is_term_screen_show");
                                }
                                if (jsonObjectSetting.has("is_report_user")) {
                                    setting.setReportUserBool(jsonObjectSetting.getBoolean("is_report_user"));
                                }
                                if (jsonObjectSetting.has("is_blocked_ads")) {
                                    setting.setBlockADBool(jsonObjectSetting.getBoolean("is_blocked_ads"));
                                }
                                ...
                                if (response.getJSONObject("extra_text").has("enter_email")) {
                                    enterEmail = response.getJSONObject("extra_text").getString("enter_email");
                                }
                                if (response.getJSONObject("extra_text").has("not_allowed_field")) {
                                    EditProfile.notAllowed = response.getJSONObject("extra_text").getString("not_allowed_field");
                                }
                                    ...
                                    handler.postDelayed(() -> {
                                        ...
                                        if (setting.getUserLogin().equals("0")) {
                                            if (setting.getAppOpen()) {
                                                ...
                                                editor.apply();
                                                if (setting.getPrivacyPolicyBool()) {
                                                    ...
                                                    if (istermsPrivacyOn) {
                                                        Intent intent = new Intent(activity, UserConsent.class);
                                                        startActivity(intent);
                                                    } else {
                                                        Intent intent = new Intent(activity, HomeActivity.class);
                                                        startActivity(intent);
